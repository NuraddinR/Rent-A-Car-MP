"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g = Object.create((typeof Iterator === "function" ? Iterator : Object).prototype);
    return g.next = verb(0), g["throw"] = verb(1), g["return"] = verb(2), typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (g && (g = 0, op[0] && (_ = 0)), _) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
var review_1 = __importDefault(require("../mongoose/schemas/review"));
var reservation_1 = __importDefault(require("../mongoose/schemas/reservation"));
var reservation_2 = require("../types/reservation");
var rent_1 = __importDefault(require("../mongoose/schemas/rent"));
var review_2 = require("../types/review");
var getAll = function (_, res) { return __awaiter(void 0, void 0, void 0, function () {
    var reviews, err_1;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                _a.trys.push([0, 2, , 3]);
                return [4 /*yield*/, review_1.default.find()
                        .populate("rent")
                        .populate("author", "name username email avatar")];
            case 1:
                reviews = _a.sent();
                res.status(200).json({
                    items: reviews,
                    message: "Reviews fetched successfully! ",
                });
                return [3 /*break*/, 3];
            case 2:
                err_1 = _a.sent();
                console.log(err_1);
                res.status(500).json({ message: "Internal server error!" });
                return [3 /*break*/, 3];
            case 3: return [2 /*return*/];
        }
    });
}); };
var getByRentId = function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var rentId, reviews, err_2;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                _a.trys.push([0, 2, , 3]);
                rentId = req.params.rentId;
                return [4 /*yield*/, review_1.default.find({
                        rent: rentId,
                        status: review_2.ReviewStatus.Approved,
                    })
                        .populate("author", "name username email avatar")
                        .populate("rent")];
            case 1:
                reviews = _a.sent();
                res.status(200).json({
                    items: reviews,
                    message: "Reviews fetched successfully! ",
                });
                return [3 /*break*/, 3];
            case 2:
                err_2 = _a.sent();
                console.log(err_2);
                res.status(500).json({ message: "Internal server error!" });
                return [3 /*break*/, 3];
            case 3: return [2 /*return*/];
        }
    });
}); };
var create = function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var user, _a, content, rate, reservationId, reservation, review, err_3;
    return __generator(this, function (_b) {
        switch (_b.label) {
            case 0:
                _b.trys.push([0, 5, , 6]);
                user = req.user;
                _a = req.matchedData, content = _a.content, rate = _a.rate, reservationId = _a.reservationId;
                return [4 /*yield*/, reservation_1.default.findById(reservationId)];
            case 1:
                reservation = _b.sent();
                if (!reservation) {
                    res.status(404).json({ message: "Reservation not found!" });
                    return [2 /*return*/];
                }
                if (reservation.dropOffDate > new Date()) {
                    res
                        .status(400)
                        .json({ message: "You can not leave a review after drop off date" });
                    return [2 /*return*/];
                }
                if (reservation.status !== reservation_2.ReservationStatus.Approved) {
                    res.status(400).json({
                        message: "You can only review after the reservation is approved",
                    });
                    return [2 /*return*/];
                }
                if (reservation.customer.toString() !== user._id.toString()) {
                    res.status(400).json({
                        message: "You can only review your own reservation",
                    });
                    return [2 /*return*/];
                }
                if (reservation.hasReview) {
                    res.status(400).json({
                        message: "You already reviewed this reservation",
                    });
                    return [2 /*return*/];
                }
                reservation.hasReview = true;
                return [4 /*yield*/, reservation.save()];
            case 2:
                _b.sent();
                return [4 /*yield*/, review_1.default.create({
                        author: user._id,
                        content: content,
                        rate: rate,
                        rent: reservation.rent,
                    })];
            case 3:
                review = _b.sent();
                return [4 /*yield*/, rent_1.default.findByIdAndUpdate(reservation.rent, {
                        $push: { reviews: review._id },
                    })];
            case 4:
                _b.sent();
                res.status(200).json({
                    item: review,
                    message: "Review created successfully!",
                });
                return [3 /*break*/, 6];
            case 5:
                err_3 = _b.sent();
                console.log(err_3);
                res.status(500).json({ message: "Internal server error!" });
                return [3 /*break*/, 6];
            case 6: return [2 /*return*/];
        }
    });
}); };
var changeStatus = function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var id, status, review;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                id = req.params.id;
                status = req.matchedData.status;
                return [4 /*yield*/, review_1.default.findById(id)];
            case 1:
                review = _a.sent();
                if (!review) {
                    res.status(404).json({ message: "Review not found!" });
                    return [2 /*return*/];
                }
                review.status = status;
                return [4 /*yield*/, review.save()];
            case 2:
                _a.sent();
                res.status(200).json({
                    item: review,
                    message: "Review status changed successfully!",
                });
                return [2 /*return*/];
        }
    });
}); };
var reviewController = {
    getAll: getAll,
    getByRentId: getByRentId,
    create: create,
    changeStatus: changeStatus,
};
exports.default = reviewController;
